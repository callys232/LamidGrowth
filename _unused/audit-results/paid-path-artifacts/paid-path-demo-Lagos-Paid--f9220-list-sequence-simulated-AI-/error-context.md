...
- Name: paid-path-demo.spec.ts >> Lagos-Paid: paid specialist sequence (simulated AI)
...
```ts
  ...
  3   | import pg from 'pg';
  4   |
  5   | // Test-only shortcut, disclosed in the results: tops up a signup's points balance directly in
  6   | // Postgres (same connection the dedicated server itself uses), so the full paid sequence can be
  ...
  71  |       const enableBox = page.getByRole('checkbox', { name: 'Allow members to request external AI reviews' });
> 72  |       if (!(await enableBox.isChecked())) await enableBox.check();
      |                             ^ TimeoutError: locator.isChecked: Timeout 30000ms exceeded.
  73  |       await page.getByRole('button', { name: 'Save AI settings' }).click();
  74  |       await expect(page.getByRole('checkbox', { name: 'Allow members to request external AI reviews' })).toBeChecked();
  75  |
