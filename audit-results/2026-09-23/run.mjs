...
import { mkdirSync, readdirSync, existsSync, copyFileSync, readFileSync, writeFileSync, createWriteStream, unlinkSync } from 'node:fs';
...
function files(dir) {
  return existsSync(dir) ? readdirSync(dir, { withFileTypes: true }).flatMap(e => e.isDirectory() ? files(resolve(dir, e.name)) : [resolve(dir, e.name)]) : [];
}
...
writeFileSync(resolve(out, 'artifact-baseline.json'), JSON.stringify(original.map(f => relative(artifacts, f)), null, 2));
const env = { ...process.env, NODE_OPTIONS: `--require=${resolve(out, 'environment.cjs').replaceAll('\\', '/')}` };
const steps = [
  ['build', 'npm.cmd', ['run', 'build'], true],
  ['backend', process.execPath, ['--test', '--test-concurrency=3', 'tests/*.test.mjs']],
  ...['stress-worker-races', 'stress-reward-claims', 'stress-spending'].map(n => [n, process.execPath, [`scripts/${n}.mjs`]]),
  ['browser', process.execPath, ['node_modules/@playwright/test/cli.js', 'test', `--output=${relative(root, out)}/browser-artifacts`]],
  ...['usability', 'paid-path', 'deep-coverage', 'human-pace', 'click-everything'].map(n => [n, process.execPath, ['node_modules/@playwright/test/cli.js', 'test', `--config=${n}.config.ts`, `--output=${relative(root, out)}/${n}-artifacts`]]),
...
try {
  ...
  for (const file of original) if (!existsSync(file)) copy(resolve(before, relative(artifacts, file)), file);
