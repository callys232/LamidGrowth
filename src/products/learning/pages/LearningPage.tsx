  ...
  return (
    <section className="panel">
      <div className="panel-heading">
        <div>
          <h2>Learning</h2>
          <span>Close a capability gap with a learning path — free or paid, self-paced or coached.</span>
        </div>
      </div>
      ...
      <section className="panel settings-card">
        ...
        {page.enrollments.length === 0 ? (
          <Empty title="No enrollments yet">Enroll in a path from the catalog below to start building capability.</Empty>
        ) : (
          <ol className="activity-feed-list">
            {page.enrollments.map((enrollment) => (
              <li key={enrollment.id} className="activity-feed-row">
                <span className="activity-feed-title">
                  ...
                  <br />
                  <small>{enrollment.progress}% complete{enrollment.due_at ? ` · due ${new Date(enrollment.due_at).toLocaleDateString()}` : ''}</small>
                </span>
                <StatusPill status={enrollment.status.replace('_', ' ')} />
                {enrollment.status === 'completed' && (
                  <Button variant="secondary" disabled={page.busy} onClick={() => void page.issueCertificate(enrollment.id)}>
                    Issue certificate
                  </Button>
                )}
              </li>
            ...
            {page.compliance.map((c) => (
              <li key={c.id} className="activity-feed-row">
                <span className="activity-feed-title">
                  ...
                  <br />
                  <small>{c.mandatory ? 'Mandatory' : 'Recommended'}{c.due_days ? ` · due within ${c.due_days} days` : ''}</small>
                </span>
                <StatusPill status={c.enrollment_status ? c.enrollment_status.replace('_', ' ') : 'not started'} />
              </li>
            ))}
          </ol>
        </section>
      ...
      <section className="panel settings-card">
        ...
        <p>Add modules afterward — a path can mix reading, video, exercises, and scored assessments.</p>
        <form onSubmit={page.createPath}>
          <Field label="Title">
            <input name="title" required maxLength={200} placeholder="e.g. Financial Modeling Basics" />
          </Field>
      ...
      <section className="panel settings-card">
        <h3>Catalog</h3>
        <p>Every published path, platform-wide — filterable by the same domain/function/industry taxonomy used for experts.</p>
              ...
              <li key={path.id} className="activity-feed-row">
                <span className="activity-feed-title">
                  ...
                  <small>
                    ...
                    {path.feedback.count > 0 ? ` · ${path.feedback.average?.toFixed(1)}★ (${path.feedback.count})` : ''}
                    {path.modules.length > 0 ? ` · ${path.modules.length} module${path.modules.length === 1 ? '' : 's'}` : ''}
                  </small>
                </span>
      ...
      <section className="panel settings-card">
        ...
        <p>Mark modules complete as you go — completing every module in a path finishes it automatically.</p>
        {page.catalog
          .filter((path) => page.enrollments.some((e) => e.path_id === path.id && e.status === 'in_progress'))
              ...
              .map(({ path, module }) => (
                <li key={module.id} className="activity-feed-row">
                  ...
                  {module.format === 'assessment' ? (
                    ...
                    >
                      <input name="score" type="number" min={0} max={100} placeholder="Score" required style={{ width: 80 }} />
                      <Button type="submit" disabled={page.busy}>
                        Submit
                      </Button>
                    </form>
                  ) : (
                    <Button disabled={page.busy} onClick={() => void page.completeModule(module.id)}>
                      Mark complete
                    </Button>
                  )}
                </li>
      ...
      <section className="panel settings-card">
        ...
        <p>Rate a path you've completed so out-of-date or unhelpful content can be found and fixed.</p>
                ...
                <li key={enrollment.id} className="activity-feed-row">
                  ...
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    <select
                      ...
                      onChange={(e) => setRatingByPath((prev) => ({ ...prev, [enrollment.path_id]: Number(e.target.value) }))}
                    >
                      {[5, 4, 3, 2, 1].map((n) => (
                    ...
                    <input
                      ...
                      onChange={(e) => setCommentByPath((prev) => ({ ...prev, [enrollment.path_id]: e.target.value }))}
                    />
                    <Button
                      disabled={page.busy}
                      onClick={() =>
                        void page.submitFeedback(enrollment.path_id, ratingByPath[enrollment.path_id] ?? 5, commentByPath[enrollment.path_id] ?? '')
                      }
                    >
                      Submit
                    </Button>
