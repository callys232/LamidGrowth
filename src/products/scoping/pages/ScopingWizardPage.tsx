...
const RISK_COPY: Record<string, string> = {
  ...
  amber: 'A few fields are still missing. Fill in what you can, or publish anyway with what is known.',
    ...
    <section className="panel">
      ...
      {page.isJurisdictionAdmin ? (
        <section className="panel settings-card">
          ...
          <p>A jurisdiction and category pair that always forces qualified review, regardless of keywords.</p>
            ...
            <ol className="activity-feed-list">
              {page.jurisdictionRules.map((rule) => (
                <li key={rule.id} className="activity-feed-row">
                  ...
                  <Button variant="secondary" disabled={page.busy} onClick={() => void page.removeJurisdictionRule(rule.id)}>
                    Remove
                  </Button>
                </li>
              ))}
      ...
      {step === 'objective' && (
        <section className="panel settings-card">
          ...
          <p>A decision, a problem, an opportunity, or a plan. “I don’t know yet” is a valid answer for everything after this.</p>
          <Field label="Objective" hint="Example: Improve customer retention">
            <input value={objective} onChange={(e) => setObjective(e.target.value)} placeholder="What outcome do you want?" />
          </Field>
          <Field label="Problem (optional)" hint="What's happening that makes this necessary? Leave blank if you're not sure yet.">
      ...
      {step === 'scope' && c && (
        <section className="panel settings-card">
          <h3>Working scope</h3>
          <p>Fill in what you know. Use “Suggest for me” for a starting point you can edit or reject.</p>
          <Field label="Desired outcome">
            <input defaultValue={c.desired_outcome} onBlur={(e) => page.update({ desiredOutcome: e.target.value })} />
          </Field>
          <Field label="Category">
            <select defaultValue={c.category ?? ''} onChange={(e) => page.update({ category: e.target.value || null })}>
          ...
          <Field label="Deliverables">
            <input defaultValue={c.deliverables} onBlur={(e) => page.update({ deliverables: e.target.value })} />
          </Field>
          <Field label="Acceptance criteria" hint="How will you know it's done and accepted?">
            <input defaultValue={c.acceptance_criteria} onBlur={(e) => page.update({ acceptanceCriteria: e.target.value })} />
          </Field>
          <Field label="In scope">
            <input defaultValue={c.in_scope} onBlur={(e) => page.update({ inScope: e.target.value })} />
          </Field>
          <Field label="Out of scope">
            <input defaultValue={c.out_of_scope} onBlur={(e) => page.update({ outOfScope: e.target.value })} />
          </Field>
          <Field label="Assumptions">
            <input defaultValue={c.assumptions} onBlur={(e) => page.update({ assumptions: e.target.value })} />
          </Field>
          <Field label="Budget context" hint="A rough figure or range is fine">
            <input defaultValue={c.budget_context} onBlur={(e) => page.update({ budgetContext: e.target.value })} />
          </Field>
          <Field label="Timeline context">
            <input defaultValue={c.timeline_context} onBlur={(e) => page.update({ timelineContext: e.target.value })} />
          </Field>
          <Field label="Jurisdiction (optional)" hint="Country or region the work is governed under, if relevant">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <Button
              ...
              onClick={async () => {
                ...
                if (!result) return;
                await page.update(
                  Object.fromEntries(Object.entries(result.suggestions).filter(([, v]) => v)) as Record<string, string>,
                );
              }}
            >
              Suggest for me
      ...
      {step === 'review' && c && (
        <section className="panel settings-card">
          ...
          <Field label="Project title">
            <input value={jobTitle} onChange={(e) => setJobTitle(e.target.value)} placeholder={c.objective} />
          </Field>
          ...
          {c.risk_band !== 'green' && !page.reviewEntry && (
            <p>
              <Button variant="secondary" disabled={page.busy} onClick={() => void page.requestReview()}>
          ...
          {page.reviewEntry && (
            <p>
              Sent to the expert review queue — <StatusPill status={page.reviewEntry.status} />. You can still publish yourself once
              you're ready.
            </p>
          )}
          <Button
