...
export function mountBilling(app, store, { paymentProvider, ecosystemAdminEmails = [] } = {}) {
  ...
  app.get('/api/admin/escrow-overview', async (req, res) => {
    if (!isAdmin(req))
      return res
        .status(403)
        .json({ error: 'Only an ecosystem administrator can view the escrow overview.' });
    const rows = await db.prepare('SELECT * FROM milestone_fundings').all();
    const sum = (status) =>
      rows.filter((r) => r.status === status).reduce((total, r) => total + r.amount_minor, 0);
    const byStatus = {};
  ...
  app.post(
    ...
    async (req, res, next) => {
      try {
        ...
        if (!concierge)
          return res.status(400).json({ error: 'This workspace has no active concierge.' });
        ...
        if (unpaid.length === 0)
          return res.status(400).json({ error: 'Nothing is currently owed to this concierge.' });
        ...
        if (!provider)
          return res.status(503).json({
            error:
              'The paystack payment provider is not configured on this server. No transfer has been made.',
          });
        if (!account)
          return res
            .status(400)
            .json({ error: 'This concierge has no active payout account registered.' });
        ...
        try {
          ...
          await transaction(async () => {
            ...
            for (const item of unpaid)
              await db
                .prepare('UPDATE billing_line_items SET paid_at = ? WHERE id = ?')
                .run(now, item.id);
            await log(
              req.workspace.id,
              req.user.name,
              'Concierge manager paid',
              concierge.user_id,
              `${unpaid.length} fee cycle(s), ${totalMinor} minor units`,
            );
          });
          res.status(201).json({
            ok: true,
            ...
            lineItemIds: unpaid.map((i) => i.id),
          });
        } catch (error) {
          logHandledError(req, res, 'concierge_payout_error', error);
          res.status(502).json({
            error:
              'The payment provider could not initiate this transfer. No fees have been marked paid.',
          });
        }
      } catch (error) {
        next(error);
