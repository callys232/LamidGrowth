...
export async function mountAccounts(
  ...
  store,
  {
    ...
    mailProvider = defaultMailer(),
    securityKey = process.env.ACCOUNT_SECURITY_KEY,
  ...
  if (securityKey && !/^[a-f0-9]{64}$/i.test(securityKey))
    throw new Error('ACCOUNT_SECURITY_KEY must be 32 random bytes encoded as 64 hex characters.');
  if (production && !securityKey)
    throw new Error('ACCOUNT_SECURITY_KEY is required in production.');
  if (!securityKey) {
    await db
      .prepare('INSERT INTO account_security_keys VALUES (?, ?) ON CONFLICT (name) DO NOTHING')
      .run('development', randomBytes(32).toString('hex'));
    securityKey = (
      await db.prepare('SELECT value FROM account_security_keys WHERE name = ?').get('development')
    ).value;
  }
  ...
  if (production && (origin.protocol !== 'https:' || origin.username || origin.password))
  ...
  await transaction(async () => {
    for (const user of await db
      .prepare(
        'SELECT id, email FROM users WHERE demo = 0 AND id NOT IN (SELECT user_id FROM signup_signals)',
      )
      .all()) {
      await db
        .prepare(
          "INSERT INTO welcome_claims VALUES (?, ?, NULL, NULL, 'legacy', 'Existing balance preserved', ?) ON CONFLICT (user_id) DO NOTHING",
        )
        .run(
          user.id,
          sign(user.email ? `email:${normalizeIdentity(user.email)}` : `legacy:${user.id}`),
          Date.now(),
        );
    }
  });
  const accountLimiter = createRateLimiter(store, {
  ...
  function device(req, res) {
    const raw = (req.headers.cookie || '')
      .split(';')
      .map((s) => s.trim())
      .find((s) => s.startsWith('lamid_device='))
      ?.slice(13);
    let id;
    if (raw && /^[a-f0-9]{64}\.[a-f0-9]{64}$/.test(raw)) {
      ...
      if (
        timingSafeEqual(
          ...
          Buffer.from(sign(`device:${candidate}`), 'hex'),
        )
      )
        id = candidate;
    }
  ...
  async function issue(user, purpose, { cooldown = true } = {}) {
    const recent = await db
      .prepare(
        'SELECT created_at FROM otp_challenges WHERE user_id = ? AND purpose = ? ORDER BY created_at DESC LIMIT 1',
      )
      .get(user.id, purpose);
    if (cooldown && recent && Date.now() - recent.created_at < 60_000)
      fail('Please wait one minute before requesting another email.', 429);
    const id = randomUUID(),
    ...
    await db
      .prepare(
        'UPDATE otp_challenges SET used_at = ? WHERE user_id = ? AND purpose = ? AND used_at IS NULL',
      )
      .run(Date.now(), user.id, purpose);
    await db
      .prepare(
        'UPDATE account_tokens SET used_at = ? WHERE user_id = ? AND kind = ? AND used_at IS NULL',
      )
      .run(Date.now(), user.id, purpose);
    // An older queued email must not arrive after a new challenge was issued.
    await db
      .prepare(
        "UPDATE mail_outbox SET status = 'superseded', payload = '' WHERE user_id = ? AND status = 'pending'",
      )
      .run(user.id);
    await db
      .prepare('INSERT INTO otp_challenges VALUES (?, ?, ?, ?, 0, ?, NULL, ?)')
      .run(id, user.id, purpose, sign(`${id}:${code}`), expires, Date.now());
    await db
      .prepare('INSERT INTO account_tokens VALUES (?, ?, ?, ?, ?, NULL, ?)')
      .run(id, user.id, purpose, digest(token), expires, Date.now());
      ...
      user.id,
      {
        ...
        subject:
          purpose === 'verification'
            ? 'Verify your LAMID ONE account'
            : 'Reset your LAMID ONE password',
        text:
          purpose === 'verification'
            ? `Your verification code is ${code}. It expires in 10 minutes.\n\nYou can also verify here: ${link}\n\nIf you did not request this, ignore this email.`
            : `Reset your password using this single-use link, valid for 10 minutes:\n${link}\n\nIf you did not request this, ignore this email.`,
      },
      expires,
    );
    return {
      ...
      ...(production
        ...
        : {
            ...(purpose === 'verification'
              ? { verificationToken: token, developmentCode: code }
              : { recoveryToken: token }),
          }),
    };
  }
  async function award(userId) {
    ...
    for (const key of [
      `welcome:ip:${signals.ip_hash}`,
      `welcome:device:${signals.device_hash}`,
      `welcome:email:${signals.email_hash}`,
    ].sort())
      await db.prepare('SELECT pg_advisory_xact_lock(hashtext(?))').get(key);
    const duplicate = await db
      .prepare(
        "SELECT 1 FROM welcome_claims WHERE email_hash = ? OR (device_hash = ? AND status = 'granted')",
      )
      .get(signals.email_hash, signals.device_hash);
    // Email identity collisions cannot insert a second unique claim. Keep a non-PII decision in the audit.
    if (
      duplicate &&
      (await db
        .prepare('SELECT 1 FROM welcome_claims WHERE email_hash = ?')
        .get(signals.email_hash))
    )
      return { status: 'ineligible', points: 0 };
    const count = (
      await db
        .prepare(
          "SELECT COUNT(*) AS n FROM welcome_claims WHERE ip_hash = ? AND status = 'granted' AND created_at > ?",
        )
        .get(signals.ip_hash, Date.now() - 86400000)
    ).n;
    const status = duplicate
      ? 'ineligible'
      : count >= welcomeIpVelocityLimit
        ? 'review'
        : 'granted';
    ...
    const inserted = await db
      .prepare(
        'INSERT INTO welcome_claims VALUES (?, ?, ?, ?, ?, ?, ?) ON CONFLICT (user_id) DO NOTHING RETURNING *',
      )
      .get(
        userId,
        signals.email_hash,
        ...
        duplicate
          ...
          : count >= welcomeIpVelocityLimit
            ? 'Shared network velocity review'
            : 'Verified first claim',
        Date.now(),
      );
    if (!inserted)
      return {
        status: (
          await db.prepare('SELECT status FROM welcome_claims WHERE user_id = ?').get(userId)
        ).status,
        points: 0,
      };
  ...
  async function credit(userId) {
    ...
    await db
      ...
      .run(welcomeRewardPoints, userId);
    await db
  ...
  async function finish(token, purpose) {
    ...
    const row = await db
      .prepare(
        'UPDATE account_tokens SET used_at = ? WHERE token_hash = ? AND kind = ? AND used_at IS NULL AND expires_at > ? RETURNING *',
      )
      .get(Date.now(), digest(token), purpose, Date.now());
    if (
      !row ||
      !(await db
        .prepare('SELECT 1 FROM users WHERE id = ? AND disabled_at IS NULL')
        .get(row.user_id))
    )
      return null;
    await db
      .prepare('UPDATE otp_challenges SET used_at = ? WHERE user_id = ? AND purpose = ?')
      .run(Date.now(), row.user_id, purpose);
    return row;
  }
  app.post('/api/auth/signup', async (req, res) => {
    ...
    try {
      challenge = await transaction(async () => {
        ...
        await db
          .prepare('INSERT INTO signup_signals VALUES (?,?,?,?,?)')
          .run(
            user,
            sign(`email:${normalizeIdentity(input.email)}`),
            deviceHash,
            sign(`ip:${req.ip}`),
            Date.now(),
          );
    ...
    res.cookie('lamid_device', req.deviceCookie, {
      httpOnly: true,
      secure: production,
  ...
  app.post('/api/auth/resend-verification', async (req, res) => {
    ...
    const user = await db
      .prepare('SELECT * FROM users WHERE email = ? AND demo = 0 AND disabled_at IS NULL')
      .get(email);
    const challenge =
      user && !user.verified_at
        ? await transaction(() => issue(user, 'verification'))
  ...
  app.post('/api/auth/verify', async (req, res) => {
    const input = z
      .union([
        ...
        z.object({ challengeId: z.string().uuid(), code: z.string().regex(/^\d{6}$/) }).strict(),
      ])
      .parse(req.body);
    const result = await transaction(async () => {
      ...
      if ('token' in input) userId = (await finish(input.token, 'verification'))?.user_id;
        ...
        const row = await db
          .prepare(
            "SELECT * FROM otp_challenges WHERE id = ? AND purpose = 'verification' AND used_at IS NULL AND expires_at > ? AND attempts < 5 FOR UPDATE",
          )
          .get(input.challengeId, Date.now());
        if (row) {
          await db
          ...
          if (
            timingSafeEqual(
              ...
              Buffer.from(sign(`${row.id}:${input.code}`), 'hex'),
            ) &&
            (await db
              .prepare('SELECT 1 FROM users WHERE id = ? AND disabled_at IS NULL')
              .get(row.user_id))
            ...
            await db
              .prepare(
                "UPDATE otp_challenges SET used_at = ? WHERE user_id = ? AND purpose = 'verification'",
              )
              .run(Date.now(), userId);
            await db
              .prepare(
                "UPDATE account_tokens SET used_at = ? WHERE user_id = ? AND kind = 'verification'",
              )
              .run(Date.now(), userId);
          }
        }
      ...
      await db
        .prepare('UPDATE users SET verified_at = COALESCE(verified_at, ?) WHERE id = ?')
        .run(Date.now(), userId);
      return award(userId);
    });
    if (!result)
      return res.status(400).json({
        error:
          'This verification code or link is invalid, expired, or has reached its attempt limit.',
      });
  ...
  app.post('/api/auth/reset-password', async (req, res) => {
    ...
    const success = await transaction(async () => {
      ...
      await db
        ...
        .run(password, Date.now(), row.user_id);
      await db.prepare('DELETE FROM sessions WHERE user_id = ?').run(row.user_id);
      await award(row.user_id);
      return true;
    });
    if (!success)
      return res.status(400).json({ error: 'This recovery link is invalid or expired.' });
    res.json({ ok: true });
  });
  ...
  app.post('/api/auth/demo', async (_req, res) => {
    if (production)
      return res.status(403).json({ error: 'Sample accounts are unavailable on this deployment.' });
