...
export function mountBooking(app, store) {
  ...
  function ownProfile(userId) {
    const profile = db.prepare('SELECT * FROM talent_profiles WHERE user_id = ?').get(userId);
  ...
  app.post('/api/booking/availability', (req, res) => {
    ...
    const id = randomUUID();
    db.prepare('INSERT INTO availability_slots VALUES (?, ?, ?, ?, ?, ?, ?)').run(
      id,
      profile.id,
      input.startAt,
      input.endAt,
    ...
    res.status(201).json(db.prepare('SELECT * FROM availability_slots WHERE id = ?').get(id));
  });

  app.get('/api/booking/availability/mine', (req, res) => {
    const profile = ownProfile(req.user.id);
    res.json(db.prepare('SELECT * FROM availability_slots WHERE profile_id = ? ORDER BY start_at').all(profile.id));
  });

  app.delete('/api/booking/availability/:id', (req, res) => {
    const profile = ownProfile(req.user.id);
  ...
  app.get('/api/booking/availability/:expertUserId', (req, res) => {
    const profile = db.prepare('SELECT id FROM talent_profiles WHERE user_id = ?').get(req.params.expertUserId);
  ...
  app.post('/api/booking/slots/:slotId/book', (req, res) => {
    ...
    const expertProfile = db.prepare('SELECT user_id FROM talent_profiles WHERE id = ?').get(slot.profile_id);
    ...
    transaction(() => {
      ...
      log(req.workspace.id, req.user.name, 'Booked availability slot', id, slot.start_at);
    });
    res.status(201).json(db.prepare('SELECT * FROM bookings WHERE id = ?').get(id));
  });

  app.get('/api/booking/mine', (req, res) => {
    res.json(
      db
        .prepare(
  ...
  app.patch('/api/booking/:id/cancel', (req, res) => {
