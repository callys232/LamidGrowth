...
const rateSchema = z
  .object({ from: currencyCode, to: currencyCode, rate: z.number().positive() })
  .strict();
...
export function mountFx(app, store) {
  ...
  app.get('/api/fx/convert', async (req, res) => {
    ...
    const row = await db
      ...
      .get(`${input.from}_${input.to}`);
    if (!row)
      return res
        .status(404)
        .json({ error: `No indicative rate is available for ${input.from} to ${input.to}.` });
    res.json({
      amount: input.amount,
      from: input.from,
      to: input.to,
