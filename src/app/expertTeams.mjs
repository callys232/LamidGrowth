...
export function mountExpertTeams(app, store) {
  ...
  async function membersOf(id) {
    return db
      .prepare('SELECT * FROM expert_team_members WHERE team_id = ? ORDER BY created_at')
      .all(id);
  }
  ...
  app.post('/api/expert-teams', async (req, res) => {
    ...
    await transaction(async () => {
      await db
        .prepare('INSERT INTO expert_teams VALUES (?, ?, ?, ?, ?)')
        .run(id, input.name, req.user.id, input.description, now);
      await db
        .prepare('INSERT INTO expert_team_members VALUES (?, ?, ?, ?, ?, ?)')
        .run(randomUUID(), id, req.user.id, 'lead', '', now);
  ...
  app.get('/api/expert-teams/mine', async (req, res) => {
    ...
    res.json(
      await Promise.all(
        teams.map(async (team) => ({ ...team, members: await membersOf(team.id) })),
      ),
    );
  });
  ...
  app.post('/api/expert-teams/:id/members', async (req, res) => {
    ...
    if (
      await db
        ...
        .get(team.id, input.userId)
    )
      return res.status(400).json({ error: 'That expert is already on the team.' });
    const id = randomUUID();
    await db
      .prepare('INSERT INTO expert_team_members VALUES (?, ?, ?, ?, ?, ?)')
      .run(id, team.id, input.userId, input.role, input.accessScope, new Date().toISOString());
  ...
  app.delete('/api/expert-teams/:id/members/:userId', async (req, res) => {
    const team = await teamFor(req.params.id);
    requireLead(team, req.user.id);
    if (req.params.userId === team.lead_user_id)
      return res.status(400).json({ error: 'The team lead cannot be removed.' });
    await db
      .prepare('DELETE FROM expert_team_members WHERE team_id = ? AND user_id = ?')
      .run(team.id, req.params.userId);
