...
export function mountConcierge(app, store, { ecosystemAdminEmails }) {
  ...
  app.post('/api/concierge/applications', (req, res) => {
    ...
    const id = randomUUID();
    transaction(() => {
      db.prepare('INSERT INTO concierge_applications VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(
        id,
        req.user.id,
        input.headline,
        input.experience,
      ...
      log(req.workspace.id, req.user.name, 'Concierge application submitted', id, input.headline);
    });
    res.status(201).json(db.prepare('SELECT * FROM concierge_applications WHERE id = ?').get(id));
  });

  app.get('/api/concierge/applications/mine', (req, res) => {
    res.json(
      db
  ...
  app.get('/api/admin/concierge-applications', (req, res) => {
    if (!isAdmin(req))
  ...
  app.patch('/api/admin/concierge-applications/:id', (req, res) => {
    if (!isAdmin(req))
      return res.status(403).json({ error: 'Only an ecosystem administrator can decide concierge applications.' });
    const input = decisionSchema.parse(req.body);
    const application = db.prepare('SELECT * FROM concierge_applications WHERE id = ?').get(req.params.id);
    ...
    transaction(() => {
      ...
      log(req.workspace.id, req.user.name, `Concierge application ${status}`, application.id, application.headline);
    });
    res.json(db.prepare('SELECT * FROM concierge_applications WHERE id = ?').get(application.id));
  });

  app.get('/api/concierge/providers', (req, res) => {
    res.json(
      db
        .prepare(
  ...
  app.post('/api/workspace/concierge', requirePermission('workspace:manage'), (req, res, next) => {
    try {
      ...
      transaction(() => {
        ...
        recordOneTimeEcosystemFee(store, req.workspace.id);
        log(req.workspace.id, req.user.name, 'Concierge assigned', input.userId, 'Elevated to workspace:manage authority.');
      });
