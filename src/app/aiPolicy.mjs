...
const deny = message => { throw Object.assign(new Error(message), { status: 403 }); };
export async function authorizeExternalAI(store, workspaceId, principalId) {
  const user = await store.db.prepare('SELECT verified_at, demo, disabled_at FROM users WHERE id = ?').get(principalId);
  if (!user || user.demo || user.disabled_at || !user.verified_at) deny('External AI requires a verified account.');
  const member = await store.db.prepare("SELECT role FROM workspace_members WHERE user_id = ? AND workspace_id = ? AND status = 'active'").get(principalId, workspaceId);
  if (!member || !['owner', 'member'].includes(member.role)) deny('Your workspace membership does not permit external AI.');
...
export async function reserveAIUsage(store, workspaceId, principalId) {
  ...
  const since = Date.parse(new Date().toISOString().slice(0, 10));
  const globalLimit = Math.min(1000, Math.max(1, Number.parseInt(process.env.AI_GLOBAL_DAILY_LIMIT || '100', 10) || 100));
  ...
  await lockAIQuota(store, workspaceId);
  if ((await store.db.prepare('SELECT COUNT(*) AS n FROM ai_usage WHERE created_at >= ?').get(since)).n >= globalLimit ||
      (await store.db.prepare('SELECT COUNT(*) AS n FROM ai_usage WHERE workspace_id = ? AND created_at >= ?').get(workspaceId, since)).n >= policy.dailyLimit) {
    throw Object.assign(new Error('The daily AI request limit has been reached.'), { status: 429 });
  }
  const id = randomUUID();
  await store.db.prepare("INSERT INTO ai_usage VALUES (?, ?, ?, ?, 'pending')").run(id, workspaceId, principalId, Date.now());
  return id;
}
...
export function scopedProvider(store, provider, workspaceId, principalId, consent) {
  ...
  return { ...provider, async review(payload) {
    if (consent !== true) deny('Confirm consent to share the relevant workspace context with external AI.');
    if (Buffer.byteLength(JSON.stringify(payload)) > 60000) throw Object.assign(new Error('Select less context for this request.'), { status: 413 });
    ...
    try {
      const result = await Promise.race([
        ...
        new Promise((_, reject) => { timer = setTimeout(() => { controller.abort(); reject(new Error('AI request timed out.')); }, 45000); }),
      ]);
      await authorizeExternalAI(store, workspaceId, principalId);
      const ids = new Set(payload.sources.map(source => source.id));
      if ((result.review.evidenceIds || []).some(id => !ids.has(id))) throw new Error('AI returned evidence outside the selected context.');
