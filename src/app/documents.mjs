...
export function mountDocuments(app, store) {
  ...
  function runFor(id, workspaceId) {
    const run = db
  ...
  app.get('/api/agent-runs/:id/pdf', (req, res, next) => {
    try {
      const run = runFor(req.params.id, req.workspace.id);
      const doc = renderAgentRunPdf(run, req.workspace, signaturesFor(run));
  ...
  app.get('/api/agent-runs/:id/signatures', (req, res, next) => {
    try {
      const run = runFor(req.params.id, req.workspace.id);
      res.json(signaturesFor(run));
    } catch (error) {
      next(error);
    }
  });

  app.post('/api/agent-runs/:id/signatures', (req, res, next) => {
    try {
      const run = runFor(req.params.id, req.workspace.id);
      ...
      transaction(() => {
        db.prepare('INSERT INTO document_signatures VALUES (?, ?, ?, ?, ?, ?, ?)').run(
          id,
          run.id,
          req.user.id,
          input.signerName,
        ...
        log(req.workspace.id, req.user.name, 'Document signed (in-app attestation)', run.id, input.signerName);
      });
      const record = db.prepare('SELECT * FROM document_signatures WHERE id = ?').get(id);
      res.status(201).json({
        ...record,
