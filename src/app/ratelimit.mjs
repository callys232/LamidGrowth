...
export function createRateLimiter(store, { windowMs, max, message, key = (req) => req.ip }) {
  const { db } = store;
  ...
  return (req, res, next) => {
    ...
    const row = upsert.get(currentKey, newResetAt, now, now, newResetAt);
    if (row.count > max) {
...
export function pruneRateLimitBuckets(store) {
  store.db.prepare('DELETE FROM rate_limit_buckets WHERE reset_at <= ?').run(Date.now());
}
