...
export function mountLearning(app, store, { ecosystemAdminEmails = [] } = {}) {
  ...
  async function pathFor(id) {
    const path = await db.prepare('SELECT * FROM learning_paths WHERE id = ?').get(id);
  ...
  app.get('/api/learning/paths/:id', async (req, res) => {
    res.json(await pathDetail(req.params.id));
  });
  ...
  app.post('/api/learning/paths/:id/assign', async (req, res) => {
    const path = await pathFor(req.params.id);
  ...
  async function enrollmentFor(id, userId) {
    const enrollment = await db.prepare('SELECT * FROM learning_enrollments WHERE id = ?').get(id);
