...
export function validateProductionConfig(env = process.env) {
  if (!/^[a-f0-9]{64}$/i.test(env.ACCOUNT_SECURITY_KEY || '')) throw new Error('Set ACCOUNT_SECURITY_KEY to 32 random bytes in hex.');
  const url = new URL(env.PUBLIC_ORIGIN || 'http://invalid');
  if (url.protocol !== 'https:' || url.username || url.password || url.pathname !== '/' || url.search || url.hash) throw new Error('Set PUBLIC_ORIGIN to the public HTTPS origin.');
  if (env.COOKIE_SECURE === 'false') throw new Error('Production cookies must be Secure.');
  if ((!env.RESEND_API_KEY && !env.SENDGRID_API_KEY) || !env.MAIL_FROM) throw new Error('Configure RESEND_API_KEY or SENDGRID_API_KEY, plus MAIL_FROM, before starting public production.');
  if (env.TRUST_PROXY_HOPS && !/^[1-9]\d*$/.test(env.TRUST_PROXY_HOPS)) throw new Error('TRUST_PROXY_HOPS must match a positive number of trusted proxy hops.');
