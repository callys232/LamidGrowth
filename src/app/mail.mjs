...
export function createMailOutbox(
  store,
  provider,
  key,
  errorLogger = (event, fields) => console.error(JSON.stringify({ event, ...fields })),
) {
  ...
  return {
    ...
    async enqueue(id, user, message, expires) {
      await db
        .prepare("INSERT INTO mail_outbox VALUES (?, ?, ?, 'pending', 0, ?, NULL, ?, ?)")
        .run(id, user, encrypt(message), Date.now(), expires, Date.now());
    },
    async tick() {
      if (!provider) return;
      ...
      for (let i = 0; i < 10; i++) {
        const row = await transaction(async () => {
          ...
          await db
            .prepare(
              "UPDATE mail_outbox SET status = 'expired', payload = '' WHERE expires_at <= ? AND status IN ('pending','sending')",
            )
            .run(now);
          ...
          const next = await db
            .prepare(
              "SELECT * FROM mail_outbox WHERE expires_at > ? AND attempts < 5 AND ((status = 'pending' AND next_at <= ?) OR (status = 'sending' AND lease_until <= ?)) ORDER BY created_at LIMIT 1 FOR UPDATE SKIP LOCKED",
            )
            .get(now, now, now);
          if (!next) return null;
          await db
            .prepare(
              "UPDATE mail_outbox SET status = 'sending', attempts = attempts + 1, lease_until = ? WHERE id = ?",
            )
            .run(now + 30000, next.id);
          return next;
        });
        if (!row) break;
        try {
          ...
          await db
            .prepare(
              "UPDATE mail_outbox SET status = 'sent', payload = '', lease_until = NULL WHERE id = ?",
            )
            .run(row.id);
        } catch (error) {
          ...
          await db
            .prepare(
              'UPDATE mail_outbox SET status = ?, next_at = ?, lease_until = NULL, payload = CASE WHEN ?::boolean THEN ? ELSE payload END WHERE id = ?',
            )
            .run(
              failed ? 'failed' : 'pending',
              Date.now() + 15000 * 2 ** row.attempts,
              failed,
              '',
              row.id,
            );
          errorLogger('mail_delivery_retry', {
            messageId: row.id,
