...
export function mountOutcomes(app, store) {
  ...
  app.post('/api/projects/:id/outcome', (req, res) => {
    ...
    if (db.prepare('SELECT 1 FROM engagement_outcomes WHERE project_id = ?').get(project.id))
      return res.status(400).json({ error: 'An outcome has already been recorded for this project.' });
    const input = createSchema.parse(req.body);
    const id = randomUUID();
    const now = new Date().toISOString();
    transaction(() => {
      db.prepare('INSERT INTO engagement_outcomes VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(
        id,
        project.id,
        req.workspace.id,
        req.user.id,
      ...
      log(req.workspace.id, req.user.name, 'Engagement outcome recorded', id, project.title);
    });
    res.status(201).json(db.prepare('SELECT * FROM engagement_outcomes WHERE id = ?').get(id));
  });

  app.get('/api/projects/:id/outcome', (req, res) => {
    const project = db.prepare('SELECT * FROM projects WHERE id = ?').get(req.params.id);
    if (!project) return res.status(404).json({ error: 'Project not found.' });
    if (project.workspace_id !== req.workspace.id) return res.status(403).json({ error: 'This project does not belong to your workspace.' });
    res.json(db.prepare('SELECT * FROM engagement_outcomes WHERE project_id = ?').get(project.id) ?? null);
  });
  ...
  app.get('/api/workspace/outcomes', (req, res) => {
    res.json(
      db
        .prepare(
