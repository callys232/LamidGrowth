...
export function mountHandoff(app, store) {
  ...
  app.post('/api/handoffs/:id/accept', async (req, res) => {
    ...
    await transaction(async () => {
      await db
        .prepare(
          "UPDATE handoffs SET status = 'accepted', target_user_id = ?, resolved_at = ? WHERE id = ?",
        )
        .run(req.user.id, new Date().toISOString(), handoff.id);
    });
  ...
  app.post('/api/handoffs/:id/complete', async (req, res) => {
    ...
    if (handoff.status !== 'accepted')
      return res.status(400).json({ error: 'Only an accepted handoff can be completed.' });
    if (handoff.target_user_id !== req.user.id)
      return res.status(403).json({ error: 'You did not accept this handoff.' });
    await db
      .prepare("UPDATE handoffs SET status = 'completed', resolved_at = ? WHERE id = ?")
      .run(new Date().toISOString(), handoff.id);
    res.json(await handoffFor(handoff.id));
  });
}
