...
export function mountScoping(app, store, { ecosystemAdminEmails } = {}) {
  ...
  function jurisdictionRequiresLicense(jurisdiction, category) {
  ...
  function caseFor(id, userId) {
    const row = db.prepare('SELECT * FROM scoping_cases WHERE id = ?').get(id);
    if (!row) fail('Scoping case not found.', 404);
    if (row.created_by !== userId) fail('You do not own this scoping case.', 403);
    return row;
  }

  app.post('/api/scoping-cases', (req, res) => {
    ...
    transaction(() => {
      ...
      log(req.workspace.id, req.user.name, 'Scoping case started', id, input.objective.slice(0, 80));
    });
    res.status(201).json(db.prepare('SELECT * FROM scoping_cases WHERE id = ?').get(id));
  });

  app.get('/api/scoping-cases', (req, res) => {
    res.json(
      db
  ...
  app.get('/api/scoping-cases/:id', (req, res) => {
    res.json(caseFor(req.params.id, req.user.id));
  });

  app.patch('/api/scoping-cases/:id', (req, res) => {
    const existing = caseFor(req.params.id, req.user.id);
    ...
    const riskBand = computeRiskBand(merged, jurisdictionRequiresLicense(merged.jurisdiction, merged.category));
    ...
    res.json(db.prepare('SELECT * FROM scoping_cases WHERE id = ?').get(req.params.id));
  });
  ...
  app.post('/api/scoping-cases/:id/suggest', (req, res) => {
    const existing = caseFor(req.params.id, req.user.id);
    const suggestions = {};
  ...
  app.patch('/api/scoping-cases/:id/publish', (req, res) => {
    const existing = caseFor(req.params.id, req.user.id);
    ...
    const job = db.prepare('SELECT id FROM job_posts WHERE id = ? AND client_user_id = ?').get(input.publishedJobId, req.user.id);
    if (!job) return res.status(400).json({ error: 'That job post was not found or was not created by you.' });
    transaction(() => {
      db.prepare("UPDATE scoping_cases SET status = 'published', published_job_id = ?, updated_at = ? WHERE id = ?").run(
        input.publishedJobId,
        new Date().toISOString(),
        req.params.id,
      );
      log(req.workspace.id, req.user.name, 'Scoping case published', req.params.id, input.publishedJobId);
    });
    res.json(db.prepare('SELECT * FROM scoping_cases WHERE id = ?').get(req.params.id));
  });
  ...
  app.post('/api/scoping-cases/:id/request-review', (req, res) => {
    const existing = caseFor(req.params.id, req.user.id);
    if (existing.risk_band === 'green') return res.status(400).json({ error: 'This scope was not flagged for review.' });
    if (db.prepare('SELECT 1 FROM review_queue_entries WHERE scoping_case_id = ?').get(existing.id))
      return res.status(400).json({ error: 'Review has already been requested for this scoping case.' });
    const id = randomUUID();
    transaction(() => {
      db.prepare('INSERT INTO review_queue_entries VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(
        id,
        existing.id,
        existing.risk_band,
        'pending',
      ...
      log(req.workspace.id, req.user.name, 'Expert review requested', id, existing.objective.slice(0, 80));
    });
    res.status(201).json(db.prepare('SELECT * FROM review_queue_entries WHERE id = ?').get(id));
  });

  app.get('/api/scoping-cases/:id/review', (req, res) => {
    caseFor(req.params.id, req.user.id);
    res.json(db.prepare('SELECT * FROM review_queue_entries WHERE scoping_case_id = ?').get(req.params.id) ?? null);
  });
  ...
  app.post('/api/review-queue/:id/claim', (req, res) => {
    if (!db.prepare('SELECT 1 FROM talent_profiles WHERE user_id = ?').get(req.user.id))
      return res.status(403).json({ error: 'Only registered experts can claim review queue entries.' });
    const entry = db.prepare('SELECT * FROM review_queue_entries WHERE id = ?').get(req.params.id);
    ...
    db.prepare("UPDATE review_queue_entries SET status = 'claimed', claimed_by = ?, claimed_at = ? WHERE id = ?").run(
      req.user.id,
      new Date().toISOString(),
      entry.id,
    );
    res.json(db.prepare('SELECT * FROM review_queue_entries WHERE id = ?').get(entry.id));
  });

  app.post('/api/review-queue/:id/complete', (req, res) => {
    const entry = db.prepare('SELECT * FROM review_queue_entries WHERE id = ?').get(req.params.id);
    if (!entry) return res.status(404).json({ error: 'Review queue entry not found.' });
    ...
    db.prepare("UPDATE review_queue_entries SET status = 'completed', notes = ?, completed_at = ? WHERE id = ?").run(
      input.notes,
      new Date().toISOString(),
      entry.id,
    );
  ...
  app.get('/api/admin/jurisdiction-rules', (req, res) => {
    if (!isAdmin(req)) return res.status(403).json({ error: 'Only an ecosystem administrator can view jurisdiction rules.' });
    res.json(db.prepare('SELECT * FROM jurisdiction_rules ORDER BY jurisdiction, category').all());
  });

  app.post('/api/admin/jurisdiction-rules', (req, res) => {
    ...
    if (db.prepare('SELECT 1 FROM jurisdiction_rules WHERE jurisdiction = ? AND category = ?').get(input.jurisdiction, input.category))
      return res.status(400).json({ error: 'A rule for this jurisdiction and category already exists.' });
    const id = randomUUID();
    db.prepare('INSERT INTO jurisdiction_rules VALUES (?, ?, ?, ?, ?, ?)').run(
      id,
      input.jurisdiction,
      input.category,
    ...
    res.status(201).json(db.prepare('SELECT * FROM jurisdiction_rules WHERE id = ?').get(id));
  });

  app.delete('/api/admin/jurisdiction-rules/:id', (req, res) => {
    if (!isAdmin(req)) return res.status(403).json({ error: 'Only an ecosystem administrator can manage jurisdiction rules.' });
    db.prepare('DELETE FROM jurisdiction_rules WHERE id = ?').run(req.params.id);
    res.status(204).end();
  });
}
