...
for (const file of fs.readdirSync('tests/browser').filter((f) => f.endsWith('.spec.ts'))) {
  ...
  source = source.replace(
    /(await page\.getByRole\('button', \{ name: 'Create your workspace'(?:, exact: true)? \}\)\.click\(\);)/g,
...
let block = source
  .slice(start, end)
  .replaceAll('response.humanHandoffRequested', 'built.humanHandoffRequested')
  .replaceAll('response.handoffId', 'built.handoffId');
