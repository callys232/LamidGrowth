...
for (let attempt = 1; attempt <= 10; attempt++) {
  ...
  const store = {
    ...
    db: {
      prepare: () => ({
        ...
        run: async (name, owner, expires_at) => {
          lease = { name, owner, expires_at };
        },
      }),
    },
  };
  const claims = await Promise.all(
    ['worker-a', 'worker-b', 'worker-c'].map((owner) =>
      acquireServiceLease(store, 'scheduler', owner, 100, 100),
    ),
  );
  results.push({
    probe: 'scheduler-overlapping-claims',
    attempt,
    ownersGranted: claims.filter(Boolean).length,
    invariantPassed: claims.filter(Boolean).length === 1,
  });
}
for (let attempt = 1; attempt <= 10; attempt++) {
  let row;
      ...
      prepare: (sql) => ({
        ...
        run: async (...args) => {
          if (sql.startsWith('INSERT'))
            row = { id: args[0], payload: args[2], status: 'pending', attempts: 0 };
...
console.log(
  JSON.stringify(
    {
      scope:
        'Controlled in-memory interleavings of actual production lease/mail functions; no database or external mail calls',
      results,
    },
    null,
    2,
