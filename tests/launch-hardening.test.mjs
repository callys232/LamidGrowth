      ...
      new Promise((resolve, reject) => {
        timer = setInterval(() => {
          if (started()) resolve();
            ...
            reject(new Error('Mock provider did not start within 60 seconds.'));
        }, 10);
      }),
      request.then((result) => {
        throw new Error(`Request ended before mock provider started: ${result.status}`);
      }),
    ]);
  } finally {
    clearInterval(timer);
...
async function fixture(t, options = {}) {
  ...
  const instance = await createApp({
    filename: ':memory:',
    production: true,
    securityKey: 'ab'.repeat(32),
    publicOrigin: 'https://lamid.example',
    mailProvider: {
  ...
  async function signup(cookie) {
    ...
    const result = await call(
      '/auth/signup',
      { name: 'Launch Test', email, password: 'launch-test-password-123', context: 'Founder' },
      cookie,
    );
    ...
    return {
      ...result,
      email,
      cookie: result.cookies.map((s) => s.split(';')[0]).join('; '),
      code: message.text.match(/code is (\d{6})/)[1],
      message,
    };
  }
  const verify = (account) =>
    call('/auth/verify', { challengeId: account.data.challengeId, code: account.code });
...
test('production signup issues no secrets, grants exactly the welcome reward once after OTP, and retains login without OTP', async (t) => {
  ...
  assert.equal(
    (
      await f.store.db
        .prepare("SELECT COUNT(*) AS n FROM points_ledger WHERE reason = 'welcome_bonus'")
        .get()
    ).n,
    1,
  );
  assert.equal(
    (await f.call('/auth/login', { email: a.email, password: 'launch-test-password-123' })).data
...
test('same signed device gets no second reward; shared-network velocity waits for review', async (t) => {
  ...
  for (let i = 0; i < 2; i++)
    assert.equal((await f.verify(await f.signup())).data.welcomeReward.points, welcomeRewardPoints);
...
test('OTP expires, locks after five wrong guesses, resends rotate both code and link', async (t) => {
  ...
  for (let i = 0; i < 5; i++)
    assert.equal(
      (await f.call('/auth/verify', { challengeId: a.data.challengeId, code: wrong })).status,
      400,
    );
  ...
  assert.equal(
    (
      await f.call('/auth/verify', {
        token: new URL(a.message.text.match(/https:\/\/\S+/)[0]).searchParams.get('token'),
      })
    ).status,
    400,
  );
  ...
  assert.equal(
    (await f.call('/auth/verify', { challengeId: resend.data.challengeId, code: a.code })).status,
    400,
  );
});
...
test('production recovery mail resets password once and revokes existing sessions', async (t) => {
  ...
  assert.equal(
    (await f.call('/auth/reset-password', { token, password: 'new-launch-password-123' })).status,
    200,
  );
  assert.equal(
    (await f.call('/auth/reset-password', { token, password: 'new-launch-password-456' })).status,
    400,
  );
  assert.equal((await f.call('/state', undefined, a.cookie)).status, 401);
  assert.equal(
    (await f.call('/auth/login', { email: a.email, password: 'new-launch-password-123' })).status,
    200,
  );
});
...
test('Companion enforces verification, workspace opt-in and consent, with one provider call for concurrent retries', async (t) => {
  ...
  const provider = {
    ...
    async review() {
      ...
      return {
        review: { summary: 'Grounded response', evidenceIds: [], assumptions: [], suggestions: [] },
      };
    },
  };
  const f = await fixture(t, { aiProvider: provider }),
    a = await f.signup();
  ...
  assert.equal(
    (await f.call('/companion/messages', { message: 'Help me', consent: true }, a.cookie)).status,
    403,
  );
  await f.verify(a);
  assert.equal(
  ...
  const body = { message: 'Help me', consent: true },
    headers = { 'Idempotency-Key': 'concurrent-launch-test' };
  ...
  assert.equal(replay.data.runId, result.data.runId);
  assert.equal(calls, 1);
});
...
test('revocation during a provider request rejects the late result and refunds once', async (t) => {
  ...
  const f = await fixture(t, {
    aiProvider: {
      ...
      async review() {
        ...
        return { review: { summary: 'late', evidenceIds: [] } };
      },
    },
  });
  ...
  await f.store.transaction(async () => {
    ...
    await f.store.db
      .prepare('UPDATE users SET points_balance = points_balance + 100 WHERE id = ?')
      .run(user.id);
    await f.store.db
      .prepare(
        "INSERT INTO points_ledger VALUES (?, ?, NULL, 100, 'test_fixture_funding', NULL, ?)",
      )
  ...
  assert.equal(
    (await f.call('/points', undefined, a.cookie)).data.balance,
    welcomeRewardPoints + 100,
  );
});
...
test('scheduler lease transfers to a replacement owner and stale agent charges refund once', async (t) => {
  ...
  const state = (await f.call('/state', undefined, a.cookie)).data,
    runId = randomUUID();
  await f.store.db
    .prepare(
      "INSERT INTO agent_runs VALUES (?, ?, ?, 'context-curator', '{}', NULL, 'running', ?, NULL)",
    )
    .run(runId, state.workspace.id, state.user.id, new Date(Date.now() - 180000).toISOString());
  await f.store.db
    .prepare('UPDATE users SET points_balance = points_balance - 1 WHERE id = ?')
    .run(state.user.id);
  await f.store.db
    .prepare("INSERT INTO points_ledger VALUES (?, ?, ?, -1, 'agent_run', ?, ?)")
    .run(randomUUID(), state.user.id, state.workspace.id, runId, Date.now());
  assert.equal(await f.agentRuntime.reconcile(), 1);
  assert.equal(await f.agentRuntime.reconcile(), 0);
  assert.equal((await f.call('/points', undefined, a.cookie)).data.balance, welcomeRewardPoints);
});
...
test('production startup requires secure deploy configuration', () => {
  ...
  const valid = {
    ACCOUNT_SECURITY_KEY: 'aa'.repeat(32),
    PUBLIC_ORIGIN: 'https://example.test',
    MAIL_FROM: 'accounts@example.test',
    RESEND_API_KEY: 'test',
