...
test('a user applies, a non-admin cannot approve, an ecosystem admin can', async () => {
  ...
  const application = await request(
    ...
    { headline: 'Senior PM, 8 years', experience: 'Managed 40+ projects.' },
    applicant,
  );
...
test('a duplicate application while pending or approved is rejected', async () => {
  ...
  const first = await request(
    '/concierge/applications',
    { headline: 'PM', experience: '' },
    applicant,
  );
  assert.equal(first.status, 201);
  const second = await request(
    '/concierge/applications',
    { headline: 'PM again', experience: '' },
    applicant,
  );
...
test('an owner can only assign an approved provider, and only one active concierge at a time', async () => {
  ...
  const pendingApp = await request(
    '/concierge/applications',
    { headline: 'Not yet approved', experience: '' },
    unapproved,
  );
  void pendingApp;
  const approvedApp = await request(
    '/concierge/applications',
    { headline: 'Approved PM', experience: '' },
    approvedProvider,
  );
  await request(
    `/admin/concierge-applications/${approvedApp.data.id}`,
    { decision: 'approve' },
    admin,
    'PATCH',
  );

  const rejectedAssign = await request(
    '/workspace/concierge',
    { userId: unapprovedState.user.id },
    owner,
  );
  ...
  const secondApp = await request(
    '/concierge/applications',
    { headline: 'Second PM', experience: '' },
    secondApplicant,
  );
  await request(
    `/admin/concierge-applications/${secondApp.data.id}`,
    { decision: 'approve' },
    admin,
    'PATCH',
  );
  const secondAssign = await request(
    '/workspace/concierge',
    { userId: secondState.user.id },
    owner,
  );
  ...
  const blockedMemberAction = await request(
    '/admin/members',
    { email: approvedState.user.email },
    approvedProvider,
  );
