...
for (const [city, locale, timezoneId, width, height, role, goal] of profiles) {
  test(`${city}: onboarding, goal and specialist value`, async ({ browser }, info) => {
    const context = await browser.newContext({ locale, timezoneId, viewport: { width, height }, reducedMotion: 'reduce' });
    const page = await context.newPage();
    const result: Record<string, unknown> = { city, locale, timezoneId, viewport: `${width}x${height}`, role, goal,
      simulated: true, stage: 'homepage', completed: [] };
    ...
    try {
      ...
      result.homeOverflow = await page.evaluate(() => document.documentElement.scrollWidth > innerWidth + 1);
      result.documentLanguage = await page.locator('html').getAttribute('lang');
      const audit = await new AxeBuilder({ page }).withTags(['wcag2a', 'wcag2aa']).analyze();
      result.accessibility = audit.violations.map(v => ({ id: v.id, impact: v.impact, count: v.nodes.length, description: v.help }));
      ...
      await page.getByLabel('Email address').fill(`regional-${info.workerIndex}-${Date.now()}@example.test`);
      ...
      result.welcomeBalance = (await (await context.request.get('http://127.0.0.1:3129/api/points')).json()).balance;
      result.stage = 'goal';
