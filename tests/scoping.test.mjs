...
test('a scoping case can start from only an objective, with no scope known yet', async () => {
  ...
  const created = await request('/scoping-cases', { objective: 'Improve customer retention', problemStatement: '' }, client);
...
test('a regulated-sounding objective is flagged red even with everything else filled in', async () => {
  ...
  const created = await request('/scoping-cases', { objective: 'Review our healthcare data-handling policy', problemStatement: '' }, client);
  assert.equal(created.data.risk_band, 'red');
  const filled = await request(
    `/scoping-cases/${created.data.id}`,
    { deliverables: 'A written policy review', category: 'Legal and compliance', budgetContext: '$2000', timelineContext: '2 weeks' },
    client,
    'PATCH',
  );
...
test('a fully-specified, non-regulated case becomes green after PATCH', async () => {
  ...
  const created = await request('/scoping-cases', { objective: 'Build a marketing dashboard', problemStatement: 'No visibility into campaign performance.' }, client);
  const filled = await request(
    `/scoping-cases/${created.data.id}`,
    {
...
test('suggest returns editable suggestions without changing the stored case', async () => {
  ...
  const created = await request('/scoping-cases', { objective: 'Launch a referral program', problemStatement: '' }, client);
...
test('a red-band case cannot publish without explicit confirmation, and only over a job the user owns', async () => {
  ...
  const created = await request('/scoping-cases', { objective: 'Provide financial advice to clients', problemStatement: '' }, client);
  assert.equal(created.data.risk_band, 'red');
  ...
  const withoutConfirm = await request(`/scoping-cases/${created.data.id}/publish`, { publishedJobId: job.data.id }, client, 'PATCH');
  assert.equal(withoutConfirm.status, 400);

  const notOwner = await request(`/scoping-cases/${created.data.id}/publish`, { publishedJobId: job.data.id, confirmed: true }, stranger, 'PATCH');
  assert.equal(notOwner.status, 403);

  const published = await request(`/scoping-cases/${created.data.id}/publish`, { publishedJobId: job.data.id, confirmed: true }, client, 'PATCH');
  assert.equal(published.status, 200);
  assert.equal(published.data.status, 'published');
  assert.equal(published.data.published_job_id, job.data.id);

  const editAfterPublish = await request(`/scoping-cases/${created.data.id}`, { objective: 'Changed' }, client, 'PATCH');
