...
export default defineConfig({
  testDir: './tests/browser', testMatch: 'paid-path-demo.spec.ts', workers: 1,
  timeout: 180000, expect: { timeout: 25000 }, reporter: 'list',
  use: { baseURL: `http://127.0.0.1:${port}`, actionTimeout: 30000, trace: 'retain-on-failure' },
  webServer: { command: 'node scripts/usability-server-paid.mjs', url: `http://127.0.0.1:${port}/api/health`, timeout: 120000, reuseExistingServer: false },
});
